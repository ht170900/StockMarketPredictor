...
#  endif
	...
	class _interpreter {
		...
		_interpreter() {
			static int argc = 1;
			static char* argv[] = { (char*)"StockMarketPredictor", NULL };
			PyConfig config;
			PyConfig_InitPythonConfig(&config);

			std::vector<std::string> args(argc);
			std::vector<char*> cstrs(argc);
			for (int i = 0; i < argc; i++) {
				args[i] = std::string(PyUnicode_AsUTF8(PyUnicode_DecodeFSDefault(argv[i])));
				cstrs[i] = args[i].data();
			}

			PyConfig_SetBytesArgv(&config, argc, cstrs.data());
...
namespace detail {
    ...
    struct select_npy_type {
        static constexpr NPY_TYPES type = NPY_NOTYPE;
    };
#ifndef WITHOUT_NUMPY
// Type selector for numpy array conversion
#ifndef MATPLOTLIBCPP_NPY_TYPE_FIX
#define MATPLOTLIBCPP_NPY_TYPE_FIX

#ifndef SELECT_NPY_TYPE_INT64_DEFINED
#define SELECT_NPY_TYPE_INT64_DEFINED

template <> struct select_npy_type<int64_t> { static constexpr NPY_TYPES type = NPY_INT64; };
template <> struct select_npy_type<uint64_t> { static constexpr NPY_TYPES type = NPY_UINT64; };

#endif // SELECT_NPY_TYPE_INT64_DEFINED

#endif // MATPLOTLIBCPP_NPY_TYPE_FIX
